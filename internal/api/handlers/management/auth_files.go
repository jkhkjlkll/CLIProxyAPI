...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	if err := geminiAuth.ValidateOAuthClient(); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	// OAuth2 configuration using exported helpers from internal/auth/gemini
	conf := &oauth2.Config{
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	if err := antigravity.ValidateOAuthClient(); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
